...
class Jcart {
	...
	*/
	private function add_item($id, $name, $price, $qty = 1, $url) {
		...
		if ($validPrice !== false && $validQty !== false) {
			...
			// If the item is already in the cart, let them know
			if($this->qtys[$id] > 0) {
			echo "<script type=\"text/javascript\">window.alert(\"You've already added this item!\")</script>";
			exit;
			}
			// This is a new item
			else {
	...
	*/
	public function display_cart() {
		...
		echo tab(3) . "<table id='checkoutTable' border='1'>\n";
		echo tab(4) . "<thead>\n";
		echo tab(5) . "<tr>\n";
